...
const GridCard = ({
  ...
  return (
    <div className="relative">
      {/* Main Card */}
      <div
        className={`relative z-10 flex h-full flex-col gap-2 rounded-3xl border ${borderColor} ${bgColor} p-4 shadow-lg ${shadowColor} `}
      >
        {/* Title */}
        <h1
          className={`text-2xl font-bold leading-tight ${textColor} line-clamp-2`}
        >
          {title}
        </h1>
        ...
        {/* Description */}
        <p
          className={`flex-grow text-lg leading-snug ${descriptionColor} line-clamp-2`}
        >
          {description}
        </p>
      ...
      {/* Background Block */}
      <div
        className={`absolute inset-0 z-0 translate-x-1 translate-y-1 rounded-3xl ${isDeath ? "bg-[#F52A2A]" : "bg-[#63C779]"} `}
      />
    </div>
  );
};
